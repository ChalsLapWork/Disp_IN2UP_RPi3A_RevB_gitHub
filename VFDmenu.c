...
//El ascii dela   Raiz Cuadrada es el decimal 251d.//ascci  251=raiz cuadrada
void displayTextoProcessor(void){                  //ascci de x=120  X=88
unsigned char i,*s,j;	
unsigned char tx[]= {251,' ',' ','x'};//" x";	 
unsigned char t0[]= "Nombre de Producto";
unsigned char t1[]= "Escribir clave de acceso:";//Contrasena";
...
unsigned short int m;
	...
	switch(vfd.menu.contexto.control){	  
	  ...
	  case SUPERVISOR:    
				          ...
				          MenuTextProc->igxc4=0;//no ha habido escritura de ascii  
						  break;     
	  default:usleep(9);//Error de ingeniria de Software
	  			mens_Warnning_Debug("Error de Software dispTextProc:648");
	          break;
	}//fin switch	
	VFDposicion(x[0],y[0]);  
	VFDserial_SendBlock(&tx[0],sizeof(tx));//display the exit control
	...
	vfd.menu.cursorx=POSX_COL1;
	vfd.menu.cursory=POSY6;
	VFDposicion(vfd.menu.cursorx,vfd.menu.cursory);
	VFDserial_SendChar('>');
